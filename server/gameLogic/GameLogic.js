...
const CollectCoin = (lobbyGameState, userId, collectedCoinLocation) => {
  if (lobbyGameState && lobbyGameState.playerStats[userId]){
    ...
    if (lobbyGameState.hasHydraCoins) {
      ...
      for (const user_Id of Object.keys(lobbyGameState.playerStats)) {
        if(lobbyGameState && lobbyGameState[user_Id]){
          totalRoundCoinsCollected += lobbyGameState.playerStats[user_Id].roundCoins;
        }
      }
...
const MovePlayer = (lobbyGameState, userId) => {
  if(lobbyGameState && lobbyGameState.gridLayout && lobbyGameState.playerStats[userId]) {
