...
const MovePlayer = (lobbyGameState, userId) => {
    ...
    if(lobbyGameState.playerStats[userId].isAlive == true){ // CAN ONLY MOVE PLAYER IF THEY ARE ALIVE
        if(lobbyGameState.playerStats[userId].isMoving.up){
            ...
            if(futurePlayerLocation == 0 || futurePlayerLocation == 2){ // If next location is not a wall, move player's location there
...
const KillPlayers = (userA, userB, lobbyGameState) => {
    ...
    playerDeathTimer[userA] = setInterval(() => {
        ...
        if(lobbyGameState.playerStats[userA].deathCountdown <= 0){
            ...
            let [playerLocationY, playerLocationX] = gameManager.GetRandomPlayerLocation(lobbyGameState.gridLayout);
